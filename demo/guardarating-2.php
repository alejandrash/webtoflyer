...
//if (($categoria!='superusuario')||($usuario='alehusson@yahoo.com.ar')) {
	...
	for ($i = 0; $i < $count; $i++) {
	        ...
	        $cadena = $cadena." ".$id_producto[$i]." ".$id_marca;
	    
	        $result=mysql_query("INSERT INTO rating_productos (id_producto, id_marca, id_cat, provincia, usuario, fecha, cara, tipo, id_sesion, legales) VALUES ('$id_producto[$i]', '$id_marca', '$id_cat', '$provincia', '$usuario', '$fecha', '$cara', '2', '$id_sesion', '$txt_legales[$i]')") or die(mysql_error());
	}
