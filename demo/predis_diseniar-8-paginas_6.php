...
// TRAE LEGALES ACTUALIZADOS
    ...
    $result_predi=mysql_query("select * from predisenio WHERE tipo='8' ORDER BY Id DESC LIMIT 1");
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='tapa' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='tapa' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='contratapa' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='contratapa' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='pag2' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='pag2' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='pag3' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='pag3' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='pag4' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='pag4' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='pag5' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='pag5' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' and cara='pag6' ORDER BY Id ASC");
        if ($row_legf=mysql_fetch_array($result_legf)) {
            do { 
                if ($row_legf['legales']!=''){
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
            ...
            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND predisenio='si' AND legales!='' and cara='pag6' AND id_sesion='$sesion_predisenio' ORDER BY Id ASC");
            if ($row_legf=mysql_fetch_array($result_legf)) {
                do { 
                    $posicion_coincidencia = strpos($resultLeg, $row_legf['legales']);
                    if ($posicion_coincidencia === false) {
    ...
    function cargarLegales() {
        ...
        $(document).ready(function () {
            ...
            if (!isset($_SESSION{"predis_pag7_8_".$_SESSION['ESTADO']})) {
                if ($niveluser!='operador') {
                    $result_esta=mysql_query("select * from rating_productos WHERE cara='pag7' and tipo='8' AND actual='si' and predisenio='si'");
                    if($row_esta=mysql_fetch_array($result_esta)){
                        $delete_esta=mysql_query("DELETE FROM rating_productos WHERE cara='pag7' and tipo='8' AND actual='si' and predisenio='si'");
                    }
                }
                if ($niveluser=='operador') {
                    $result_esta=mysql_query("select * from rating_productos WHERE cara='pag7' and tipo='8' AND actual='si' and usuario='$usuario'");
                    if($row_esta=mysql_fetch_array($result_esta)){
                        $delete_esta=mysql_query("DELETE FROM rating_productos WHERE cara='pag7' and tipo='8' AND actual='si'");
                    }
                }
            }
            ...
            if (isset($_SESSION{"predis_pag7_8_".$_SESSION['ESTADO']})) { ?>
                ...
                var content = '<?php $_SESSION{"predis_pag7_8_".$_SESSION['ESTADO']}; ?>';
            ...
            $(".guardar").click(function(e) { 
                    ...
                    if (isset($_SESSION{"predis_archivo_pag7_8_".$_SESSION['ESTADO']})) { ?>
                        var name = '<?php echo($_SESSION{"predis_archivo_pag7_8_".$_SESSION['ESTADO']}); ?>';
                    <?php 
                    }
                    else {
                    ?>
                                ...
                                if (isConfirm) {
                                        ...
                                        $.ajax({
                                                ...
                                                success: function(data){
                                                    ...
                                                    <?php
                                                        if ($niveluser!='operador') {
                                                            $location = "predis_diseniar-8-paginas_detalle_central.php";
                                                        }
                                                        if ($niveluser=='operador') {
                                                            $location = "predis_diseniar-8-paginas_detalle.php";
                                                        }
                                                    ?>
                            ...
                            <div class="row">
                                <div class="editar_partes">
                                    <ul>
                                        ...
                                        <li><a id="ed_7" class="disabled activada" href="predis_diseniar-8-paginas_6.php" title="EDITAR PAGINA 7"><strong>7</strong></a></li>
                                    </ul>
                                    ...
                                    <p class="boton"><a title="CONTINUAR AL SIGUIENTE PASO" class="continuar disabled" href="#"><i class="fa fa-check-square-o" aria-hidden="true"></i> GUARDAR Y FINALIZAR</a></p>
                                    <?php
                                    if ($niveluser=='operador') {
                                    ?>
                                        ...
                                        <div id="container-legales">
                                                <div class="inner-legales"> 
                                                    ...
                                                    <div class="legales-financiacion">
                                                        ...
                                                        <?php
                                                            $sess = $_SESSION['SESIONFLY'];
                                                            $result_legf2=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='8' AND id_sesion='$sess' AND legales!='' ORDER BY Id ASC");
                                                            if ($row_legf2=mysql_fetch_array($result_legf2)) {
                                                                do {
                                                        ?>
                                                                    <span><?php echo($row_legf2['legales']); ?></span>
                                                            <?php
                                                                } while ($row_legf2=mysql_fetch_array($result_legf2));    
                                                            }
                                                             ?>
