...
?>
<?php
    $result_traecantidad=mysql_query("select * from banners_flyer WHERE cara='contratapa' AND mes='$mesbanner'");
    $valor = 0;
    $valortotal = 0;
    $maximoprod = 9;
...
// CONSULTA BLOQUEO DE MARCAS
    $result_bloqueo=mysql_query("select marca, cantidad, nombre, logo from bloqueo INNER JOIN marcas ON marcas.Id=bloqueo.marca WHERE cara='contratapa' order by fecha desc");
?>
<!DOCTYPE HTML>
<html>
<head>
 ...
 <script type="text/javascript">   
    ...
    function cargarLegales() {
        ...
        $(document).ready(function () {
            ...
            //	esBloqueado = false;
            //	if ($(this).parent().hasClass('bloqueado')) {
            //		esBloqueado = true;
            //		bloqueoMarca = $(this).parent().attr('data-marca');
            //	}  
            //    var contenido = $('#wraprod').html();
            //    if (contenido == '') {
            //    	if (esBloqueado == false) {
	        //            $("#wraprod").load( "productos-inner.php", function() {
	         //             $('#btndown').fadeIn();
	         //           });
             //   	}
             //   	if (esBloqueado == true) {
             //   		$("#wraprod").load( "productos-inner_pormarca.php?bloqueoMarca="+bloqueoMarca, function() {
	        //              $('#btndown').fadeIn();
	         //           });
             //   	}
           ...
           // });
            ...
            if (!isset($_SESSION{"contratapa_".$_SESSION['ESTADO']})) {
                $result_esta=mysql_query("select * from rating_productos WHERE usuario='$usuario' and cara='contratapa' and tipo='16' and actual='si'");
                if($row_esta=mysql_fetch_array($result_esta)){
                    $delete_esta=mysql_query("DELETE FROM rating_productos WHERE usuario='$usuario' and cara='contratapa' and tipo='16' and actual='si'");
                }
            }
            ...
            $(".continuar").click(function(e) {
                    ...
                    if (valUno=='') {
                        hayerror = 1;
                        swal("Error", "Debe seleccionar por lo menos UNA DIRECCION DE SUCURSAL.", "error");
                        $("html, body").animate({
                        ...
                        $('.direccion_elegir .form-control').each(function() {
                            if ( $.inArray(this.value, values) >= 0 ) {
                                if (this.value!=0) {
                                    hayerror = 1;
                                    swal("Error", "Hay sucursales repetidas en los desplegables. Seleccione diferentes sucursales en cada desplegable o elimine las sucursales que no desee.", "error");
                                    $("html, body").animate({
                                        scrollTop: $("#end").offset().top
                                    }, 800);
                                    return false;
                    ...
                    if (hayerror == 0) {
                        ...
                        $('#flyercontainer .quitar, #flyercontainer .eliminar-sda, #flyercontainer #agregar-sda img, #flyercontainer .frases, #flyercontainer .selecucarda span, #flyercontainer .direccion_elegir .form-control, #flyercontainer .direccion_elegir i.fa-minus-circle, #flyercontainer .direccion #agregarsuc, #flyercontainer .tachado .quitartachado, #flyercontainer .agregartachado').remove();
                        ...
                        $.ajax({
                                ...
                                success: function(data){
                                    $('#cargando').fadeOut();
                                    window.location.href = "diseniar-16-paginas_1.php";
                                }
                        });
                    }
            });
            ...
            $('#flyercontainer').on('change', '.direccion_elegir .form-control', function() { 
                if ($(this).val()=='') {
                    ...
                    if ($(this).attr('id')=='direccion_elegir_1') {
                        ...
                        $(this).parent().children().children('.direccion_resultante').children('.load_telefono').html('<i class="fa fa-phone" aria-hidden="true"></i> '+loadTel);
                    }
                    else {
            ...
            $('#flyercontainer').on('click', '.direccion #agregarsuc', function() { 
                ...
                if (addSuc>6) {
                    swal("Error", "Podés agregar hasta seis (6) sucursales por flyer.", "error");
                    return false;
                }
                if (TotalSuc <= VisibleSuc) {
                    swal("Error", "No hay mas sucursales para agregar.", "error");
                }
                else {
                    var numAdd = (VisibleSuc + 2);
                    if (VisibleSuc > 0) {
                        $('#block-direcciones .direccion_resultante').css('top', '5px');
                    }
                    if (VisibleSuc > 1) {
                        $('.direccion_elegir').removeClass('col-sm-offset-2');
                        $('.direccion_elegir').removeClass('col-sm-8');
                        $('.direccion_elegir').addClass('col-sm-6');
                        $('#block-direcciones .direccion_resultante').css('font-size', '16px');
                    }
                    else {
                    ...
                    if (VisibleSuc > 3) {
                        $('#block-direcciones .direccion_resultante').css('top', '-20px');
                        $('#block-direcciones .direccion_resultante').css('height', '58px');
                    }
                    $('.direccion > .direccion_elegir:nth-child(' + numAdd + ')').css('display','block');
                    if ((VisibleSuc == 2) || (VisibleSuc == 4)) {
                        $('.direccion_elegir:visible:last').addClass('col-sm-offset-2');
                        $('.direccion_elegir:visible:last').addClass('col-sm-8');
                        $('.direccion_elegir:visible:last').removeClass('col-sm-6');
                    }
            ...
            $('#flyercontainer').on('click', '.direccion_elegir i.fa-minus-circle', function() { 
                ...
                if (VisibleSuc > 3) {
                        ...
                        $('.direccion_elegir').addClass('col-sm-offset-2');
                        $('.direccion_elegir').addClass('col-sm-8'); 
                        $('#block-direcciones .direccion_resultante').css('font-size', '20px'); 
                }
                if (VisibleSuc > 5) {
                    $('#block-direcciones .direccion_resultante').css('top', '-20px');
                    $('#block-direcciones .direccion_resultante').css('height', '58px');
                    ...
                    $('#block-direcciones .direccion_resultante').css('top', '5px');
                    $('#block-direcciones .direccion_resultante').css('height', '70px');
                }
                if ((VisibleSuc == 4) || (VisibleSuc == 6)) {
                    $('.direccion_elegir:visible:last').addClass('col-sm-offset-2');
                    $('.direccion_elegir:visible:last').addClass('col-sm-8');
                    $('.direccion_elegir:visible:last').removeClass('col-sm-6');
                }
                if (VisibleSuc < 3) {
                    $('#block-direcciones .direccion_resultante').css('top', '38px');
                }
	   ...
	   <div class="inner-content">
		<!-- header-starts -->
           ...
           <div class="col-xs-12 titulogris">
                <p class="col-xs-12"><img src="images/titulos/diseniar.png" class="img-responsive" alt=""> DISEÑAR - Editando CONTRATAPA</p>
            </div>
			<!-- Diseniar start-->
                        ...
                        <div id="lateral" class="lateral col-lg-4 col-md-4 col-sm-12 col-xs-12 pull-right">
                            <div class="row">
                                <div class="editar_partes paginas16">
                                    <ul>
                                        <li><a id="ed_tapa" class="disabled" href="diseniar-16-paginas.php" title="EDITAR TAPA">TAPA</a></li>
                                        <li><a id="ed_contratapa" class="disabled activada" href="diseniar-16-paginas_contratapa.php" title="EDITAR CONTRATAPA">CONTRATAPA</a></li>
                        ...
                        <!--START FLYER-->
                                <div id="flyercontainer">
                                    <div id="contratapa" class="wrap-flyer">
                                        ...
                                        <div id="block-direcciones" class="col-xs-12">
                                                <div class="txtizq col-xs-6 pull-left">ENCONTR&Aacute; ESTAS OFERTAS EN:</div>
                                                <div class="txtder col-xs-6 pull-left text-right">www.tumarca.com</div>
                                                <div class="direccion">
                                        ...
                                        <div class="bordebottom" id="end"><div class="logopie"><img src="images/diseniar/logopie.png" width="167" alt=""></div><div class="legalespie">Los precios contado son v&aacute;lidos en efectivo, tarjeta de d&eacute;bito &oacute; tarjeta de cr&eacute;dito en 1 pago. Consulte por otros planes de financiaci&oacute;n. </div></div>
...
<div id="ayuda-despl">
    ...
    <div class="col-xs-12 contenedor">
        ...
        <ul>
            <li>
                <div class="pregunta"><span>■</span> Quisiera agregar otra sucursal ¿Qué hago?</div>
                <div class="respuesta">
                    Primero debes seleccionar la primer sucursal que aparecerá en el flyer, si querés agregar otra, apretá el botón que dice “agregar otra sucursal al flyer”. En caso de que la sucursal que necesitas no aparezca en el listado, comunícate con nosotros al (011) 4871.8184 ó escribirnos a través del chat online para que la agreguemos luego de chequear la información con Casa Central.
                </div>
            </li>
            <li>
            ...
            <li>
                <div class="pregunta"><span>■</span> ¿Cómo hago para volver a la tapa?</div>
                <div class="respuesta">
