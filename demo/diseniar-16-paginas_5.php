...
?>
<?php
    $result_traecantidad=mysql_query("select * from banners_flyer WHERE cara='pag6' AND mes='$mesbanner'");
    $valor = 0;
    $valortotal = 0;
    $maximoprod = 9;
...
// CONSULTA BLOQUEO DE MARCAS
    $result_bloqueo=mysql_query("select marca, cantidad, nombre, logo from bloqueo INNER JOIN marcas ON marcas.Id=bloqueo.marca WHERE cara='pag6' order by fecha desc");
?>
<!DOCTYPE HTML>
<html>
<head>
    ...
    function cargarLegales() {
        ...
        $(document).ready(function () {
            ...
            $('#promopie').on('click', ".modal-body .col-xs-12 img", function() { 
                ...
                $('.promospie').html('<img src="'+pieUrl +'" alt="" height="228" style="width:100%; float:left; position:absolute; top:0; left:0;" title="Click para modificar el banner">');
            ...
            if (!isset($_SESSION{"pag6_".$_SESSION['ESTADO']})) {
                $result_esta=mysql_query("select * from rating_productos WHERE usuario='$usuario' and cara='pag6' and tipo='16' and actual='si'");
                if($row_esta=mysql_fetch_array($result_esta)){
                    $delete_esta=mysql_query("DELETE FROM rating_productos WHERE usuario='$usuario' and cara='pag6' and tipo='16' and actual='si'");
                }
            }
            ...
            if (isset($_SESSION{"pag6_".$_SESSION['ESTADO']})) { ?>
                ...
                var content = '<?php $_SESSION{"pag6_".$_SESSION['ESTADO']}; ?>';
                $('.editar_partes #ed_pag6').removeClass('disabled');
                $.ajax({
                       type: "POST",
                       dataType: "html",
                       url: "recuperarpag6.php",
                       data: {
                            'content': content
                        },
                       success: function(data)
            ...
            $(".guardar").click(function(e) { 
                    ...
                    <?php 
                    if (isset($_SESSION{"archivo_pag6_".$_SESSION['ESTADO']})) { ?>
                        var name = '<?php echo($_SESSION{"archivo_pag6_".$_SESSION['ESTADO']}); ?>';
                    <?php 
                    }
                    else {
                    ?>
                    var name = '<?php echo("flyer/html/FLYER_PAG6_".date('m-d-Y_hia').".html"); ?>';
                    <?php 
                    }
                    ?>
            ...
            $(".continuar").click(function(e) {
                    ...
                    $("#flyercontainer .producto-final .zona-cuota:visible input").each(function() {
                        if ($(this).val() == '$0,00') {
                            controlPCuota = 1;
                        } 
                    });
                    ...
                    if (controlpie == 'CLICK PARA SELECCIONAR UN BANNER...') {
                        hayerror = 1;
                        swal("Error", "Debe seleccionar un banner para la cabecera de página.", "error");
                        $("html, body").animate({
                    ...
                    if (hayerror == 0) {
                        ...
                        $.ajax({
                                ...
                                success: function(data){
                                    ...
                                    window.location.href = "diseniar-16-paginas_6.php";
                                }
                        });
                    }
            });
	   ...
	   <div class="inner-content">
		<!-- header-starts -->
           ...
           <div class="col-xs-12 titulogris">
                <p class="col-xs-12"><img src="images/titulos/diseniar.png" class="img-responsive" alt=""> DISEÑAR - Editando P&Aacute;GINA 6</p>
            </div>
			<!-- Diseniar start-->
                ...
                <div class="col-lg-12 col-md-12 col-xs-12">
                            ...
                            <div class="row">
                                <div class="editar_partes paginas16">
                                    <ul>
                                        ...
                                        <li><a id="ed_6" class="disabled activada" href="diseniar-16-paginas_5.php" title="EDITAR PAGINA 6"><strong>6</strong></a></li>
                    ...
                    <!--END LATERAL-->
                        <?php 
                            ...
                            $result_promo6=mysql_query("select * from promos_pag6 WHERE visible='0' OR visible='$sessionId' ORDER BY Id DESC");
                        ?>
                        <!--START FLYER-->
                        <div id="flyercontainer">
                                    <div id="pag6" class="wrap-flyer">
                                        ...
                                        <div class="grupoproductos col-xs-12">
                                           <div id="block1" class="un-cuarto">
                                            ...
                                            <div id="block4" class="un-cuarto">
                                                <div class="row promo_pag6">
                                                    <div href="#promopie" data-toggle="modal" class="promos col-xs-12 promospie" style="height:228px;">CLICK PARA SELECCIONAR UN BANNER...</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="bordebottom" id="end"><div class="logopie"><span>6</span></div><div class="legalespie">Los precios contado son v&aacute;lidos en efectivo, tarjeta de d&eacute;bito &oacute; tarjeta de cr&eacute;dito en 1 pago. Consulte por otros planes de financiaci&oacute;n. </div></div>
    ...
    <div class="modal-content" style="height:100%; overflow:auto;">
      ...
      <div class="modal-body" style="padding-top:30px!important;">
            <?php
                $sessionId = $_SESSION['IDUSER'];
                $result_promo6=mysql_query("select * from promos_pag6 WHERE visible='0' OR visible='$sessionId' ORDER BY Id DESC"); 
                if ($row_promo6=mysql_fetch_array($result_promo6)) {
                    $select_us=mysql_query("select * from usuarios WHERE email='$usuario'");
                    $row_usua=mysql_fetch_array($select_us);
                    $id_usua=$row_usua['Id'];

                    do { 
                        $visible=$row_promo6['visible'];
                        if ($visible == '0') {
                    ?>
                        <div class="col-xs-12"><img src="flyer/<?php echo($row_promo6['foto']);?>" class="img-responsive"></div>
                    <?php
                        }
                        else {
                    ...
                    } while ($row_promo6=mysql_fetch_array($result_promo6));  
                }
            ?>
      </div>
      <div class="modal-footer">
