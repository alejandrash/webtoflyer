...
$result_rs=mysql_query("select * from usuarios WHERE email='$user'");
...
<script type="text/javascript">
    $(document).ready(function () { 
        ...
        $('#clave').change(function () {
            var estado = $('#clave2').parent('.form-group').css('display');
            if (estado=='none') {
                $('#clave2').parent('.form-group').removeClass('hidden');
            }
        });
	   ...
	   <div class="inner-content">
		<!-- header-starts -->
            ...
            <div class="col-xs-12 titulogris">
                        <p class="col-xs-12"><img src="images/titulos/editardatos.png" class="img-responsive" alt="">Editar Datos</p>
            </div>
            <div class="wrapper-prod-general">
                <!-- Estadisticas start-->
                <div class="wrap-container col-xs-12">
                    <div id="wrap-editar" class="col-md-9 col-sm-12 col-xs-12">
                        <form action="editardatos_procesar.php" method="post" id="editar-datos" name="editar-datos" class="col-xs-12" autocomplete="off">
                            <input type="hidden" name="id_usuario" id="id_usuario" value="<?php echo($row_rs['Id']); ?>">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control disabled" type="text" name="razonsocial" id="razonsocial" value="<?php //echo($row_rs['sucursal']); ?>" disabled>
                            </div>-->
                            <!--
                            <div class="form-group">
                                <label for="pvp">% PVP de Lista</label>
                                <input class="form-control" type="text" name="pvp" id="pvp" value="<?php echo($row_rs['pvp']); ?>" required onkeypress="return Onlynumbers(event)">
                            </div>-->
                            <div class="form-group relative">
                                    <img src="images/loading.gif" class="loading" height="20" alt="">
									<label for="sucursal">Seleccione la sucursal que quiere editar</label>
                                    <select  class="form-control" id="sucursal" name="sucursal">
                                        <option value="">Seleccione...</option>
                                        <?php
                                            $id_rs=$row_rs['Id'];
                                             $result_sucs=mysql_query("select Id, nombre_sucursal from sucursales WHERE id_usuario='$id_rs' ORDER BY nombre_sucursal ASC");
                                            if ($row_sucs=mysql_fetch_array($result_sucs)) {
                                                do {
                                        ?>
                                        <option value="<?php echo($row_sucs['Id']); ?>"><?php echo($row_sucs['nombre_sucursal']); ?></option>
                                        <?php
                                                } while ($row_sucs=mysql_fetch_array($result_sucs));	
                                            }
                                        ?>
                                    </select>
				            </div>
                            ...
                            <div class="form-group">
                                    ...
                                    <input type="email" class="form-control" id="email" name="email" style="text-transform: lowercase;" placeholder="E-mail" value="<?php echo($row_rs['email']); ?>" required>
								</div>
                                <div class="form-group">
