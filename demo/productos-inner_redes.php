            ...
            <div class="wrapper-prod-general">
                ...
                <div class="wrap-categorias col-lg-2 col-md-12 col-xs-12">
                    <ul>
                        ...
                        ?>
                        <li><a class="active" href="productos-inner_redes.php">Todos</a></li>
                        <?php
                        }
                        else {
                        ?>
                        <li><a href="productos-inner_redes.php">Todos</a></li>
                        <?php    
                        }
                        ?>
                        <?php 
                        ...
                        ?>
                        <li><a class="<?php echo($clase); ?>" href="productos-inner_redes.php?id_cat=<?php echo($row_cat['Id']); ?>"><?php echo($row_cat['nombre']); ?></a></li>
                        <?php
                                $clase = '';
                ...
                <div class="wrap-productos col-lg-10 col-md-12 col-xs-12">
                                        ...
                                        if (isset($_GET['id_cat'])) {
                                            ...
                                            ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function () {
                                                        ...
                                                        $('.ordenar-por li:nth-child(2) a').addClass('active');
                                                    });
                                                </script>
                                            <?php
                                            }
                                            if ($query == 'marca') {
                                                if ($id_cat!=''){
                                                    ...
                                                    $num_total_registros = mysql_num_rows($select_prod);
                                                    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
                                                    $select_prod= "select productos.Id, productos.id_cat, productos.id_marca, productos.titulo, productos.descripcion, productos.modelo, productos.foto, productos.precio, productos.origen, marcas.nombre from productos INNER JOIN marcas on productos.id_marca=marcas.Id WHERE id_cat='$id_cat' and productos.Id not in (select ".$nombre_tabla.".Id from ".$nombre_tabla.") ORDER BY marcas.nombre ASC LIMIT ".$inicio."," . $TAMANO_PAGINA;
                                                    $select_prod = mysql_query($select_prod);
                                                }
                                                else {
                                                    $select_prod=mysql_query("select productos.Id, productos.id_cat, productos.id_marca, productos.titulo, productos.descripcion, productos.modelo, productos.foto, productos.precio, productos.origen, marcas.nombre from productos INNER JOIN marcas on productos.id_marca=marcas.Id where productos.Id not in (select ".$nombre_tabla.".Id from ".$nombre_tabla.") ORDER BY marcas.nombre ASC");
                                                    $num_total_registros = mysql_num_rows($select_prod);
                                                    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
                                                    $select_prod= "select productos.Id, productos.id_cat, productos.id_marca, productos.titulo, productos.descripcion, productos.modelo, productos.foto, productos.precio, productos.origen, marcas.nombre from productos INNER JOIN marcas on productos.id_marca=marcas.Id where productos.Id not in (select ".$nombre_tabla.".Id from ".$nombre_tabla.") ORDER BY marcas.nombre ASC LIMIT ".$inicio."," . $TAMANO_PAGINA;
                                                    $select_prod = mysql_query($select_prod);
                                            ...
                                            ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function () {
                                                        $('.ordenar-por li a').removeClass('active');
                                                        $('.ordenar-por li:nth-child(3) a').addClass('active');
                                                    });
                                                </script>
                                            <?php
                                            }
                                            ...
                                            if ($query == 'marca') {
                                                ...
                                                $num_total_registros = mysql_num_rows($select_prod);
                                                $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
                                                $select_prod= "select * from productos ORDER BY marcas.nombre ASC LIMIT ".$inicio."," . $TAMANO_PAGINA;
                                                $select_prod = mysql_query($select_prod);
                                            ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function () {
                    ...
                    <div class="ordenar-por col-xs-12">
                        <div class="row">
                            <ul>
                                <li>Ordenar por:</li>
                                <li>// <a class="active" href="productos-inner_redes.php?id_cat=<?php echo($id_cat);?>&query=id">carga reciente</a></li>
                                <li>// <a href="productos-inner_redes.php?id_cat=<?php echo($id_cat);?>&query=marca">marca</a></li>
                            </ul>
                        </div></ul>
                    </div>

                    <div class="slider-paginacion" id="carousel-pag">
                        <span class="prev"><i class="fa fa-chevron-circle-left" aria-hidden="true"></i></span>
                        <div class="paginacion" style="top:0; padding:5px 5px 15px 5px;">
                            <?php
                            if ($total_paginas > 1) {
                                if (!isset($_GET['id_cat'])) {
                                   $id_cat = '';
                               ...
                               if ($pagina != 1) 
                                  echo '<a href="productos-inner_redes.php?id_cat='.$id_cat.'&query='.$query.'&pagina='.($pagina-1).'">&laquo;</a>';
                                  for ($i=1;$i<=$total_paginas;$i++) {
                                     if ($pagina == $i)
                                        ...
                                        echo '  <a href="productos-inner_redes.php?id_cat='.$id_cat.'&query='.$query.'&pagina='.$i.'">'.$i.'</a>  ';
                                  }
                                  if ($pagina != $total_paginas)
                                     echo '<a href="productos-inner_redes.php?id_cat='.$id_cat.'&query='.$query.'&pagina='.($pagina+1).'">&raquo;</a>';
                            }
                            ?>
                        </div>
                        <span class="next"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i></span>
                    </div>
