...
$select_prod=mysql_query("select productos.Id AS id_prod, productos.id_cat, productos.id_marca, productos.titulo, productos.modelo, productos.descripcion, productos.foto, productos.precio, productos.usuario, productos.tags, marcas.nombre, marcas.logo from productos inner join marcas ON productos.id_marca = marcas.Id WHERE (titulo LIKE '%$buscar%' OR modelo LIKE '%$buscar%' OR descripcion LIKE '%$buscar%' OR tags LIKE '%$buscar%' OR marcas.nombre LIKE '%$buscar%') ORDER BY productos.Id DESC");
