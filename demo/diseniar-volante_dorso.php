...
?>
<?php
    $result_traecantidad=mysql_query("select * from banners_flyer WHERE cara='prueba' AND mes='$mesbanner'");
    $valor = 0;
    $valortotal = 0;
    $maximoprod = 6;
...
// CONSULTA BLOQUEO DE MARCAS
    $result_bloqueo=mysql_query("select marca, cantidad, nombre, logo from bloqueo INNER JOIN marcas ON marcas.Id=bloqueo.marca WHERE cara='prueba' order by fecha desc");
?>
...
// TRAE LEGALES ACTUALIZADOS
    ...
    $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='2' AND id_sesion='$sess' and cara='frente' ORDER BY Id ASC");
 ...
 <script type="text/javascript">   
    ...
    function cargarLegales() {
        ...
        $(document).ready(function () {
            ...
            /*START Datepicker Legales */
                            ...
                            var checkin = $('#flyercontainer #dpd1').datepicker({
                              ...
                              if (ev.date.valueOf() > checkout.date.valueOf()) {
                                ...
                                //$("#flyercontainer #dpd1").val(checkin.date);
                                //$("#flyercontainer #dpd1").attr('value',  checkin.date);
                              }
                              checkin.hide();
            ...
            <?php
            if (!isset($_SESSION{"dorso_".$_SESSION['ESTADO']})) {
                $result_esta=mysql_query("select * from rating_productos WHERE usuario='$usuario' and cara='dorso' and tipo='2' and actual='si'");
                if($row_esta=mysql_fetch_array($result_esta)){
                    $delete_esta=mysql_query("DELETE FROM rating_productos WHERE usuario='$usuario' and cara='dorso' and tipo='2' and actual='si'");
                }
            }
            if (isset($_SESSION{"frente_".$_SESSION['ESTADO']})) { ?>
                $('.editar_partes #ed_tapa').removeClass('disabled');
            <?php }
            
            if (isset($_SESSION{"dorso_".$_SESSION['ESTADO']})) { ?>
            
                var content = '<?php $_SESSION{"dorso_".$_SESSION['ESTADO']}; ?>';
                $('.editar_partes #ed_contratapa').removeClass('disabled');
                $.ajax({
                       type: "POST",
                       dataType: "html",
                       url: "recuperardorso.php",
                       data: {
                            'content': content
                        },
                       success: function(data)
            ...
            $(".guardar").click(function(e) {
                    ...
                    <?php 
                    if (isset($_SESSION{"archivo_dorso_".$_SESSION['ESTADO']})) { ?>
                        var name = '<?php echo($_SESSION{"archivo_dorso_".$_SESSION['ESTADO']}); ?>';
                    <?php 
                    }
                    else {
                    ?>
                    var name = '<?php echo("flyer/html/FLYER_DORSO_".date('m-d-Y_hia').".html"); ?>';
                    <?php 
                    }
                    ?>
            ...
            $(".continuar").click(function(e) {
                    ...
                    if (controlFechasLegales == 1) {
                        hayerror = 1;
                        swal("Error", "Complete las FECHAS DE VALIDEZ de las ofertas.", "error");
                        return false;
                    }
                    if (controlBloqueo != 0) {
                        hayerror = 1;
                        ...
                        swal({
                            ...
                            function(isConfirm) {
                                if (isConfirm) {
                                        ...
                                        $('#flyercontainer .quitar, #flyercontainer .eliminar-sda, #flyercontainer #agregar-sda img, #flyercontainer .frases, #flyercontainer .selecucarda span, #container-legales input, #flyercontainer .direccion_elegir .form-control, #flyercontainer .direccion_elegir i.fa-minus-circle, #flyercontainer .direccion #agregarsuc, #flyercontainer .tachado .quitartachado, #flyercontainer .agregartachado').remove();
                                        ...
                                        $.ajax({
                                                ...
                                                success: function(data){
                                                    $('#cargando').fadeOut();
                                                    window.location.href = "diseniar-volante_detalle.php";
                                                }
                                        });
                                }
	   ...
	   <div class="inner-content">
		<!-- header-starts -->
           ...
           <div class="col-xs-12 titulogris">
                <p class="col-xs-12"><img src="images/titulos/diseniar.png" class="img-responsive" alt=""> DISEÑAR - Editando Dorso</p>
            </div>
			<!-- Diseniar start-->
                        ...
                        <div id="lateral" class="lateral col-lg-4 col-md-4 col-sm-12 col-xs-12 pull-right">
                            <div class="row">
                                <div class="editar_partes">
                                    <ul>
                                        <li style="display: inline-block; float:none!important; margin-right: 0!important;"><a id="ed_tapa" class="disabled" href="diseniar-volante.php" title="EDITAR FRENTE">FRENTE</a></li>
                                        <li style="display: inline-block; float:none!important; margin-right: 0!important;"><a id="ed_contratapa" class="disabled activada" href="diseniar-volante_dorso.php" title="EDITAR DORSO">DORSO</a></li>
                                    </ul>
                    ...
                    <!--END LATERAL-->
                        <!--START FLYER-->
                                <div id="flyercontainer">
                                    <div id="dorso" class="wrap-flyer">
                                        <div class="aramis"><?php echo($aramis);?></div>
                                        ...
                                        <div id="container-legales">
                                                <div class="inner-legales"> 
                                                    ...
                                                    <div class="legales-financiacion">
                                                        ...
                                                        <?php
                                                            $sess = $_SESSION['SESIONFLY'];
                                                            $result_legf=mysql_query("select DISTINCT legales from rating_productos WHERE tipo='2' AND id_sesion='$sess' AND legales!='' ORDER BY Id ASC");
                                                            if ($row_legf=mysql_fetch_array($result_legf)) {
                                                                do {
                                                        ?>
